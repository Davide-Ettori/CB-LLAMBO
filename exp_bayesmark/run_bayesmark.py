...
if __name__ == '__main__':
    ...
    parser.add_argument('--sm_eval_mode', type=str, default='mc',
                        help='Surrogate eval mode: mc | bandit_ucb1 | bandit_ucb1_tuned')
    ...
    sm_eval_mode = args.sm_eval_mode
    ...
    for seed in range(num_seeds):
        ...
        # mark max/min score rows for easy inspection
        search_history['max'] = 0
        search_history['min'] = 0
        if not search_history.empty and 'score' in search_history.columns:
            max_idx = search_history['score'].idxmax()
            min_idx = search_history['score'].idxmin()
            search_history.loc[max_idx, 'max'] = 1
            search_history.loc[min_idx, 'min'] = 1
